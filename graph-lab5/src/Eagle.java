...
public class Eagle extends JFrame {
    ...
    static TransformGroup wholeEagle;
    ...
    private void addModelToUniverse() throws IOException{
        scene = getSceneFromFile("source_folder//eagle.obj");
    ...
    private void setEagleElementsList() {
        nameMap=scene.getNamedObjects();
        //Print elements of your model:
        printModelElementsList(nameMap);

        wholeEagle = new TransformGroup();

        transform3D = new Transform3D();
        transform3D.rotX(-Math.PI / 2);
        wholeEagle.setTransform(transform3D);
        transform3D.setTranslation(new Vector3f(0, -1.3f, 0));
        wholeEagle.setTransform(transform3D);
        transform3D.setScale(0.5f);
        wholeEagle.setTransform(transform3D);

        root.removeChild(nameMap.get("eagle"));
        wholeEagle.addChild(nameMap.get("eagle"));
        wholeEagle.setCapability(TransformGroup.ALLOW_TRANSFORM_WRITE);
        root.addChild(wholeEagle);
    }
    ...
    private void addAppearance(){
        Appearance eagleAppearance = new Appearance();
        eagleAppearance.setTexture(getTexture("source_folder//eagleTexture.jpg"));
        TextureAttributes texAttr = new TextureAttributes();
        texAttr.setTextureMode(TextureAttributes.COMBINE);
        eagleAppearance.setTextureAttributes(texAttr);
        eagleAppearance.setMaterial(getMaterial());
        Shape3D eagle = nameMap.get("eagle");
        eagle.setAppearance(eagleAppearance);
    }
